...
#ifndef GAMES_SALOON_HPP
#define GAMES_SALOON_HPP
...
public:
    ...
    virtual std::string get_game_name() const noexcept override { return "Saloon"; }
    ...
    static Base_game* instance(Base_game* init = nullptr)
    {
        ...
        if(!the_game)
        {
            throw Unknown_error("Saloon instance is nullptr(??\?)");
        }
        else if(the_game && init && the_game != init)
        {
            throw Unknown_error("Saloon created twice(??\?)");
        }
        return the_game;
    }
};
