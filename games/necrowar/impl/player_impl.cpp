...
bool Player_::spawn_unit(const std::string& type)
{
...
bool Player_::spawn_worker(const std::string& type)
{
...
Player_::Player_(std::initializer_list<std::pair<std::string, Any&&>> init) :
    Game_object_{
        ...
        {"mana", Any{std::decay<decltype(mana)>::type{}}},
    ...
    mana(variables_["mana"].as<std::decay<decltype(mana)>::type>()),
...
void Player_::resize(const std::string& name, std::size_t size)
{
    if(name == "homeBase")
    {
        auto& vec = variables_["homeBase"].as<std::decay<decltype(home_base)>::type>();
        vec.resize(size);
        return;
    }
    else if(name == "side")
...
void Player_::change_vec_values(const std::string& name, std::vector<std::pair<std::size_t, Any>>& values)
{
    if(name == "homeBase")
    {
        using type = std::decay<decltype(home_base)>::type;
        auto& vec = variables_["homeBase"].as<type>();
        for(auto&& val : values)
        { 
