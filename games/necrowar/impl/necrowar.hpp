...
#ifndef GAMES_NECROWAR_HPP
#define GAMES_NECROWAR_HPP
...
public:
    ...
    virtual std::string get_game_name() const noexcept override { return "Necrowar"; }
    ...
    static Base_game* instance(Base_game* init = nullptr)
    {
        ...
        if(!the_game)
        {
            throw Unknown_error("Necrowar instance is nullptr(??\?)");
        }
        else if(the_game && init && the_game != init)
        {
            throw Unknown_error("Necrowar created twice(??\?)");
        }
        return the_game;
    }
};
