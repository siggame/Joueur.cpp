...
void Chess::Game::deltaUpdateField(const std::string& fieldName, boost::property_tree::ptree& delta)
{
    ...
    {
        this->files = this->gameManager->unserializeInt(delta);
    }
    ...
    {
        this->ranks = this->gameManager->unserializeInt(delta);
    }
    else if (fieldName == "session")
    {
        this->session = this->gameManager->unserializeString(delta);
    }
    else if (fieldName == "turnsToStalemate")
    {
        this->turnsToStalemate = this->gameManager->unserializeInt(delta);
    }
}
