...
#ifndef GAMES_CATASTROPHE_HPP
#define GAMES_CATASTROPHE_HPP
...
class Catastrophe : public Game_
{
public:
    Catastrophe() :
        Game_{}
    ...
    virtual std::string get_game_name() const noexcept override { return "Catastrophe"; }
    ...
    static Base_game* instance(Base_game* init = nullptr)
    {
        ...
        if(!the_game)
        {
            throw Unknown_error("Catastrophe instance is nullptr(??\?)");
        }
        else if(the_game && init && the_game != init)
        {
            throw Unknown_error("Catastrophe created twice(??\?)");
        }
        return the_game;
    }
};
