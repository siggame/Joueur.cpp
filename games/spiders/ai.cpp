...
// This is a simple function to return a random element of an data structure (e.g. std::vector)
template <typename T>
...
bool Spiders::AI::runTurn()
{
    ...
    if(spider->gameObjectName == "BroodMother")
    {
        ...
        if(rand() % 10 == 1) // 10% of the time try to consume a Spiderling
        {
            ...
            if(broodMother->nest->spiders.size() > 1) // there is another spider
            {
                ...
                if(otherSpider != broodMother)
                {
                    ...
                    // dynamic_cast will return nullptr. Always check for this.
        ...
        else // try to spawn a Spiderling
        {
            ...
            if(broodMother->eggs > 0)
            {
                ...
                // This is the "new" auto specifier.
                ...
                // just make it that type".
                // It's important to understand that "auto" is not a real type.
                // Instead, it's just instructing the compiler to automatically
                // fill out what the type will be.
    ...
    else // it is a spiderling
    {
        ...
        if(spiderling->busy == "") // then it is NOT busy
        {
            ...
            if(choice == 0)
            {
                ...
                if(spiderling->nest->webs.size() > 0) { // then there is a web to move on
                    auto web = randomElement(spiderling->nest->webs);
                    cout << spiderling->gameObjectName << " #"<< spiderling->id
                         << " moving onto " << web->gameObjectName
                         << " #" << web->id << endl;
                    spiderling->move(web);
                }
            }
            else if(choice == 1)
            {
                ...
                if(spiderling->nest->spiders.size() > 1) // there is a target
                {
                    ...
                    if(otherSpider->owner != spiderling->owner) // then it's an enemy!
                    {
            ...
            else
            {
                ...
                else if(spider->gameObjectName == "Cutter")
                {
                    ...
                    if(cutter->nest->webs.size() > 0) // then cut one of them
                    {
    ...
    return true; // tell the server we are done with our turn
}



// Now here's how we actually select a random element from a data structure
template <typename T>
T randomElement(std::vector<T> v) {
    T nullResult;
