...
#ifndef GAMES_JUNGLE_HPP
#define GAMES_JUNGLE_HPP
...
public:
    ...
    virtual std::string get_game_name() const noexcept override { return "Jungle"; }
    ...
    static Base_game* instance(Base_game* init = nullptr)
    {
        ...
        if(!the_game)
        {
            throw Unknown_error("Jungle instance is nullptr(??\?)");
        }
        else if(the_game && init && the_game != init)
        {
            throw Unknown_error("Jungle created twice(??\?)");
        }
        return the_game;
    }
};
