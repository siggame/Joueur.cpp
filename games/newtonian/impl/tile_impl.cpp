...
Tile_::Tile_(std::initializer_list<std::pair<std::string, Any&&>> init) :
    Game_object_{
        ...
        {"machine", Any{std::decay<decltype(machine)>::type{}}},
    ...
    machine(variables_["machine"].as<std::decay<decltype(machine)>::type>()),
...
void Tile_::rebind_by_name(Any* to_change, const std::string& member, std::shared_ptr<Base_object> ref)
{
   if(member == "machine")
   { 
      to_change->as<Machine>() = std::static_pointer_cast<Machine_>(ref);
      return;
   }
   if(member == "owner")
   { 
