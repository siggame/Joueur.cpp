...
bool Unit_::act(const Tile& tile)
{
    ...
    order += "}}}";
    Newtonian::instance()->send(order);
    //Go until not a delta
    std::unique_ptr<Any> info;
    //until a not bool is seen (i.e., the delta has been processed)
    do
    {
        info = Newtonian::instance()->handle_response();
...
Unit_::Unit_(std::initializer_list<std::pair<std::string, Any&&>> init) :
    Game_object_{
        ...
        {"blueium", Any{std::decay<decltype(blueium)>::type{}}},
        {"blueiumOre", Any{std::decay<decltype(blueium_ore)>::type{}}},
        ...
        {"redium", Any{std::decay<decltype(redium)>::type{}}},
        {"rediumOre", Any{std::decay<decltype(redium_ore)>::type{}}},
        {"stunImmune", Any{std::decay<decltype(stun_immune)>::type{}}},
    ...
    blueium(variables_["blueium"].as<std::decay<decltype(blueium)>::type>()),
    blueium_ore(variables_["blueiumOre"].as<std::decay<decltype(blueium_ore)>::type>()),
    ...
    redium(variables_["redium"].as<std::decay<decltype(redium)>::type>()),
    redium_ore(variables_["rediumOre"].as<std::decay<decltype(redium_ore)>::type>()),
    stun_immune(variables_["stunImmune"].as<std::decay<decltype(stun_immune)>::type>()),
