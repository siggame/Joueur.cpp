...
#ifndef GAMES_NEWTONIAN_HPP
#define GAMES_NEWTONIAN_HPP
...
public:
    ...
    virtual std::string get_game_name() const noexcept override { return "Newtonian"; }
    ...
    static Base_game* instance(Base_game* init = nullptr)
    {
        ...
        if(!the_game)
        {
            throw Unknown_error("Newtonian instance is nullptr(??\?)");
        }
        else if(the_game && init && the_game != init)
        {
            throw Unknown_error("Newtonian created twice(??\?)");
        }
        return the_game;
    }
};
