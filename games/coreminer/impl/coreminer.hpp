...
#ifndef GAMES_COREMINER_HPP
#define GAMES_COREMINER_HPP
...
public:
    ...
    virtual std::string get_game_name() const noexcept override { return "Coreminer"; }
    ...
    static Base_game* instance(Base_game* init = nullptr)
    {
        ...
        if(!the_game)
        {
            throw Unknown_error("Coreminer instance is nullptr(??\?)");
        }
        else if(the_game && init && the_game != init)
        {
            throw Unknown_error("Coreminer created twice(??\?)");
        }
        return the_game;
    }
};
